...
int IMC20948::begin()
{
	I2CC.Start();			  // starting the I2C bus
	//_i2c->setClock(_i2cRate); // setting the I2C clock

	if (changeUserBank(USER_BANK_0, true) < 0)
	...
	readMagRegisters(MAG_HXL, MAG_DATA_LENGTH, _buffer); // instruct the IMC20948 to get data from the magnetometer at the sample rate
	return 1;
}

int IMC20948::enableI2cMaster()
{
...
int IMC20948::enableDataReadyInterrupt()
{
	if (changeUserBank(USER_BANK_0) < 0)
	{
		return -1;
	}
	if (writeRegister(UB0_INT_PIN_CFG, UB0_INT_PIN_CFG_HIGH_50US) < 0)
	{ // setup interrupt, 50 us pulse
		return -2;
	}
	if (writeRegister(UB0_INT_ENABLE_1, UB0_INT_ENABLE_1_RAW_RDY_EN) < 0)
	{ // set to data ready
		return -3;
	}
	return 1;
...
int IMC20948::disableDataReadyInterrupt()
{
	if (changeUserBank(USER_BANK_0) < 0)
	{
		return -1;
	}
	if (writeRegister(UB0_INT_ENABLE_1, UB0_INT_ENABLE_1_DIS) < 0)
	{ // disable interrupt
		return -1;
	}
	return 1;
...
int IMC20948::selectAutoClockSource()
{
...
int IMC20948::configAccel(AccelRange range, AccelDlpfBandwidth bandwidth)
{
...
int IMC20948::configGyro(GyroRange range, GyroDlpfBandwidth bandwidth)
{
	if (changeUserBank(USER_BANK_2) < 0)
	{
		return -1;
...
int IMC20948::setGyroSrd(uint8_t srd)
{
...
int IMC20948::setAccelSrd(uint16_t srd)
{
	if (changeUserBank(USER_BANK_2) < 0)
	{
		return -1;
...
float IMC20948::getAccelX_mss()
{
	return _ax;
}

/* returns the accelerometer measurement in the y direction, m/s/s */
float IMC20948::getAccelY_mss()
{
	return _ay;
}

/* returns the accelerometer measurement in the z direction, m/s/s */
float IMC20948::getAccelZ_mss()
{
	return _az;
}

/* returns the gyroscope measurement in the x direction, rad/s */
float IMC20948::getGyroX_rads()
{
	return _gx;
}

/* returns the gyroscope measurement in the y direction, rad/s */
float IMC20948::getGyroY_rads()
{
	return _gy;
}

/* returns the gyroscope measurement in the z direction, rad/s */
float IMC20948::getGyroZ_rads()
{
	return _gz;
}

/* returns the magnetometer measurement in the x direction, uT */
float IMC20948::getMagX_uT()
{
	return _hx;
}

/* returns the magnetometer measurement in the y direction, uT */
float IMC20948::getMagY_uT()
{
	return _hy;
}

/* returns the magnetometer measurement in the z direction, uT */
float IMC20948::getMagZ_uT()
{
	return _hz;
}

/* returns the die temperature, C */
float IMC20948::getTemperature_C()
{
	return _t;
}
...
int IMC20948::changeUserBank(UserBank userBank)
{
	return changeUserBank(userBank, false);
}

int IMC20948::changeUserBank(UserBank userBank, bool force)
{
...
int IMC20948::writeMagRegister(uint8_t subAddress, uint8_t data)
{
	if (changeUserBank(USER_BANK_3) < 0)
	{
		return -1;
...
int IMC20948::readMagRegisters(uint8_t subAddress, uint8_t count, uint8_t *dest)
{
	if (changeUserBank(USER_BANK_3) < 0)
	{
		return -1;
...
/* writes a byte to IMC20948 register given a register address and data */
int IMC20948::writeRegister(uint8_t subAddress, uint8_t data)
{
	...
	I2CC.Write_Single_Register(_address,subAddress,data);
...
/* reads registers from IMC20948 given a starting register address, number of bytes, and a pointer to store data */
int IMC20948::readRegisters(uint8_t subAddress, uint8_t count, uint8_t *dest)
{
	uint8_t aui8ReadBuffer[count];
	//_i2c->beginTransmission(_address); // open the device
	//_i2c->write(subAddress);		   // specify the starting register address
	//_i2c->endTransmission(false);
	//_numBytes = _i2c->requestFrom(_address, count); // specify the number of bytes to receive
	
	I2CC.Read_Multiple_Registers(_address,subAddress,count,aui8ReadBuffer);
		
		for (uint8_t i = 0; i < count; i++)
		{
			dest[i] = aui8ReadBuffer[i];
		}
		return 1;
	//else
	//{
